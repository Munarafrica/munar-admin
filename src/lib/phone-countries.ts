...
function getDialDigits(country: PhoneCountry) {
  return country.dialCode.replace(/\D/g, "");
}

function getPhoneDigits(phone: string) {
  const digits = phone.replace(/\D/g, "");
  return phone.trim().startsWith("00") ? digits.replace(/^00/, "") : digits;
}
...
export function findPhoneCountryByDialCode(
  ...
  if (!phone.trim().match(/^(\+|00)/)) {
    return undefined;
  }

  const digits = getPhoneDigits(phone);
  const preferredCountry = preferredIso2 ? getPhoneCountryByIso2(preferredIso2) : undefined;

  if (preferredCountry && digits.startsWith(getDialDigits(preferredCountry))) {
    return preferredCountry;
  }

  return [...PHONE_COUNTRIES]
    .sort((a, b) => getDialDigits(b).length - getDialDigits(a).length)
    .find((country) => digits.startsWith(getDialDigits(country)));
}

export function ensurePhoneHasDialCode(phone: string, country: PhoneCountry) {
  const digits = getPhoneDigits(phone);
  ...
  if (digits.startsWith(dialDigits)) {
    return `${country.dialCode}${digits.slice(dialDigits.length)}`;
  }

  return `${country.dialCode}${digits.replace(/^0+/, "")}`;
}

export function extractNationalPhoneDigits(phone: string, country: PhoneCountry) {
  const digits = getPhoneDigits(phone);
  const dialDigits = getDialDigits(country);
...
export function formatInternationalPhone(phone: string, country: PhoneCountry) {
  const nationalDigits = extractNationalPhoneDigits(phone, country);
