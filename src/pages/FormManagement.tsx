...
type ConfirmActionState =
  | { kind: "archive"; form: Form }
  | { kind: "delete"; form: Form }
  | null;
...
export const FormManagement: React.FC<FormManagementProps> = ({
  ...
  const handleEditForm = (form: Form) => {
    if (form.status === "archived") {
      toast.info(
        "Archived forms are read-only. Duplicate the form if you want to reuse it.",
      );
      return;
    }
    setCurrentForm(form);
  ...
  const handlePublishForm = async (form: Form) => {
    if (form.status === "published") {
      ...
      if (updated)
        toast.success("Form published! It is now accepting responses.");
    }
  };
  ...
  const handleArchiveForm = async (form: Form) => {
    if (form.status === "archived") {
      toast.info("This form is already archived.");
      return;
    }
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (confirmAction.kind === "archive") {
        const updated = await archiveFormHook(confirmAction.form.id);
        if (updated) {
      ...
      const deleted = await deleteFormHook(confirmAction.form.id);
      if (deleted) {
        ...
        eventsService.updateModuleCount(
          ...
          `Deleted form "${confirmAction.form.title}"`,
          "file-text",
        );
        setConfirmAction(null);
      } else {
        toast.error(
          "Unable to delete form. Forms with submissions must be archived instead.",
        );
      }
    } finally {
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 text-center p-8">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 max-w-xs mt-1 mb-4">
                    Create registrations, surveys, or custom forms to collect
                    data.
                  </p>
                  <Button
                    onClick={handleCreateClick}
                            ...
                            <div className="flex flex-col">
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                <button
                                  ...
                                  className="shrink-0 rounded-md p-1 text-slate-400 opacity-0 transition-all hover:text-indigo-600 group-hover:opacity-100 dark:hover:text-indigo-400"
                          ...
                          <td className="py-4 px-4 text-right">
                            <div className="flex items-center justify-end gap-1.5">
                              <button
                                ...
                                className={cn(
                                  "p-1.5 transition-colors disabled:opacity-40 disabled:cursor-not-allowed",
                                  form.status === "published"
                              ...
                              <button
                                ...
                                disabled={form.status === "archived"}
                                className="p-1.5 text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                                title="Archive"
                              >
            ...
            <div className="p-6 space-y-4">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    Collect attendee details, sell tickets, and manage RSVPs.
                  </p>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    Collect feedback, run polls, or gather post-event insights.
                  </p>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    Start from scratch for any other data collection needs.
                  </p>
                </div>
              </button>
            </div>
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950">
            <div className="border-b border-slate-100 px-6 py-8 dark:border-slate-800">
              ...
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                {confirmAction.kind === "archive"
                  ? `Archive "${confirmAction.form.title}"? Archived forms become read-only and disappear from public listings.`
                  : `Delete "${confirmAction.form.title}"? Forms with submissions cannot be deleted.`}
              </p>
            </div>
            <div className="flex items-center justify-end gap-3 px-6 py-4">
              ...
              <Button
                ...
                disabled={isConfirming}
                className={confirmAction.kind === "archive" ? "bg-indigo-600 text-white hover:bg-indigo-700" : ""}
              >
                {isConfirming
                  ? confirmAction.kind === "archive"
                    ? "Archiving..."
