...
import { PublicTicketsResponse, CreateTicketOrderRequest, TicketOrderResponse } from '../../types/api';
...
export function TicketsPublic() {
  ...
  // Confirmation state
  const [orderResult, setOrderResult] = useState<TicketOrderResponse | null>(null);
  ...
  const handleCheckout = async () => {
    if (!isAuthenticated) {
      navigate('/login', { state: { from: { pathname: `/e/${slug}/tickets` } } });
      return;
    }
    ...
    try {
      const items = Object.entries(selectedTickets)
        ...
        .map(([ticketTypeId, quantity]) => ({
          ticketTypeId,
          quantity,
          attendeePayloads: Array.from({ length: quantity }).map(() => ({
      ...
      const checkoutData: CreateTicketOrderRequest = {
        email: buyerEmail.trim(),
        items,
        ...(answersPayload.length > 0 ? { metadataJson: { questionAnswers: answersPayload, buyerName: buyerName.trim(), buyerPhone: buyerPhone.trim() || undefined } } : {}),
      };

      const order = await ticketsService.createOrder(currentEvent!.id, checkoutData);
        ...
        <div className="max-w-2xl mx-auto px-4 py-12">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Confirmation sent to <strong className="text-slate-700 dark:text-slate-200">{orderResult.email}</strong>
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 mb-6">
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div>
                ...
                <p className="text-lg font-bold text-slate-900 dark:text-slate-100 font-mono">{orderResult.id}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-slate-500 dark:text-slate-400">Total</p>
                <p className="text-lg font-bold text-slate-900 dark:text-slate-100">
                  {orderResult.totalMinor === 0 ? 'Free' : formatCurrency(orderResult.totalMinor / 100)}
                </p>
              </div>
            </div>

            <div className="mb-4">
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{displayName}</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{displayDate} {displayTime && `at ${displayTime}`}</p>
            </div>
          </div>

          {/* Individual Tickets with QR Codes */}
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-4">Your Tickets ({orderResult.items.reduce((sum, item) => sum + item.attendees.length, 0)})</h2>
          <div className="space-y-4">
            {orderResult.items.flatMap((item) => item.attendees.map((attendee) => ({ attendee, item }))).map(({ attendee, item }) => (
              <div key={attendee.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col sm:flex-row items-center gap-5">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <p className="font-semibold text-slate-900 dark:text-slate-100">{attendee.fullName || buyerName}</p>
                  ...
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-2 font-mono">Ref: {orderResult.id}</p>
                </div>
              </div>
            ))}
          </div>
