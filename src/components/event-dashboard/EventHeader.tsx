...
export const EventHeader: React.FC<EventHeaderProps> = ({
  ...
  const publicEventUrl = event.slug ? `${window.location.origin}/e/${event.slug}` : "";
  const hasLiveWebsite = !!event.slug && !!event.websitePublished;
  const websiteBuilderUrl = `/events/${event.id}/website`;
      ...
      <div className="flex-1 flex flex-col justify-between py-1">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge
              variant="secondary"
              className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 font-medium rounded-md px-2.5 py-0.5 border-0"
            >
              {event.status}
            </Badge>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-slate-500 dark:text-slate-400">
            ...
            {hasLiveWebsite ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 hover:underline"
              >
              ...
              <span className="flex items-center gap-2 text-amber-600 dark:text-amber-400">
                <Globe className="w-4 h-4" />
                <span>Website draft only. Publish it in Website Builder to activate the public link.</span>
              </span>
            ) : event.websiteUrl ? (
              <a
        ...
        {/* Actions Toolbar */}
        <div className="flex flex-wrap items-center gap-4 mt-6 pt-5 border-t border-slate-100 dark:border-slate-800 md:mt-7 md:pt-6">
          <Button
            variant="outline"
          ...
          <Button
            ...
            onClick={() => {
              ...
              window.location.assign(websiteBuilderUrl);
            }}
          >
            <Eye className="w-4 h-4" />
            {hasLiveWebsite ? "Preview" : "Open Website Builder"}
          </Button>
          {event.status === "published" ? (
            ...
            <Button
              className="bg-indigo-600 hover:bg-indigo-700 text-white h-9 px-6 shadow-sm shadow-indigo-200 dark:shadow-none"
              onClick={onPublish}
            >
              Publish Event
            </Button>
